...
abstract class BaseViewModel : ViewModel() {
    ...
    */
     ...
     *
     * @param activity Any Activity that extends [BaseToolbarActivity]
    ...
    inline fun <reified T : IViewModelEvents> observeEventDispatcher(
        ...
        eventDispatcher.observe(activity, Observer<IViewModelEvents> {
            if (it !== null) {
                handleEvent(it as T)
    ...
    inline fun <reified T : IViewModelEvents> observeEventDispatcher(
        fragment: Fragment,
        crossinline handleEvent: (event: T) -> Unit) {
        eventDispatcher.observe(fragment, Observer<IViewModelEvents> {
            if (it !== null) {
                handleEvent(it as T)
                eventDispatcher.value = null
            }
